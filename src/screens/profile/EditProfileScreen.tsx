...
const EditProfileScreen: React.FC<{ navigation: { goBack: () => void } }> = ({ navigation }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // ensureRemotePhotoURL is a no-op for already-uploaded https URLs.
      const finalPhotoURL = await ensureRemotePhotoURL(photoURL, `users/${user.uid}/profile`);
      if (photoURL && !finalPhotoURL) {
        Alert.alert('Error', 'Failed to upload photo.');
        setLoading(false);
        return;
      }
      if (finalPhotoURL !== photoURL) {
...
const styles = StyleSheet.create({
  ...
  changePhoto: { fontSize: 16, fontWeight: '600' },
  fieldHint: { fontSize: 14, marginTop: 4, marginBottom: 4, paddingHorizontal: 4 },
